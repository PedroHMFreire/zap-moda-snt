...
export async function startSender() {
    ...
    await boss.work('send-message', async (job) => {
        const { session_id, to, text, media_url, conversation_id, store_id, message_id } = job?.data || {};
        ...
        try {
            ...
            if (!sock)
                throw new Error('session socket not found');
            if (text) {
                ...
                if (!r.ok)
                    throw new Error(`media download failed ${r.status}`);
            ...
            try {
                await boss.publish('send-message', job.data, { retryLimit: 3, retryDelay: 5000 });
            }
            catch { }
            if (message_id) {

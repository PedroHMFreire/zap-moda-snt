...
export async function ensureLocalAuthDir(sessionId) {
    const dir = authDir(sessionId);
    await fs.promises.mkdir(dir, { recursive: true });
    return dir;
}
export async function downloadAuthDirFromStorage(sessionId) {
...
export async function uploadAuthDirToStorage(sessionId) {
    const bucket = process.env.SESSION_STORAGE_BUCKET;
    if (!bucket)
        return;
    const sb = getSb();
